#same as tf_2_circles_goal.py except with with walls. reaches deadlock
#!/usr/bin/env python
...
def dontCrash(goal, time_i): 
    ...
    G = np.zeros((dim + num_cf + 4*num_cf, num_cf*3))
    x = 0
    ...
    h = np.zeros(dim + num_cf + 4*num_cf)
    x=0
...
if __name__ == "__main__":
    try:
        ...
        while i < num_rounds:
            ...
            goal = goalMotion(totalTime = [4, -1.7], radius = [1.3, 2.2], time_i =i)
